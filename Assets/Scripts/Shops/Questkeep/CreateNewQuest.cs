...
public class CreateNewQuest {
    ...
    public Quest ReturnQuest() {
        CreateQuest();
        Debug.Log("Quest { (Name-" + newQuest.QuestName + ")--(Alliance-" + newQuest.QuestAlliance.KingName + ") }");
        return newQuest;
    }
    ...
    private void ControlEvent() {
        newQuest = new Quest {
            QuestType = Quest.QuestTypes.Control
        };

        int randTemp = Random.Range(0, WorldInformation.Kingdoms.Count);

        newQuest.QuestAlliance = WorldInformation.Kingdoms[randTemp];
        newQuest.QuestLocation = WorldInformation.Areas.Find(x => x.AreaType != Area.AreaTypes.City && x.DefendingKingdom != newQuest.QuestAlliance);
        newQuest.QuestEnemy = newQuest.QuestLocation.DefendingKingdom;
    ...
    private void CreateQuest() {
        ...
        DetermineLocation();
        DetermineType();
        newQuest.QuestAlliance = WorldInformation.Kingdoms.Find(x => x.KingdomID == newQuest.QuestLocation.DefendingKingdom.KingdomID);
        DetermineName();
